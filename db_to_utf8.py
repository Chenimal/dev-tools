...
ignored_field_types = ['int', 'decimal', 'datetime', 'date',
                       'tinyint', 'smallint', 'float', 'timestamp', 'polygon', 'geometry']
...
def toUtf8(tables):
    ...
    cur.execute("ALTER DATABASE `" +
                db_config['db'] + "` DEFAULT CHARACTER SET utf8")
    i = 0
    for table_name in tables:
        i += 1
        print(Fore.WHITE + str(i) + " | " +
              table_name[0].ljust(50) + " | " + str(table_name[1]).ljust(10), end='| ')
        t1 = time.time()
        ...
        cur.execute("ALTER TABLE `" +
                    table_name[0] + "` CONVERT TO CHARACTER SET utf8")
        has_warning = cur.execute("SHOW WARNINGS")
        if has_warning > 0:
            warnings = cur.fetchmany(has_warning)
            for w in warnings:
    ...
    for i in tables:
        ...
        for row in rows:
            ...
            if not skip:
                ...
                vc = cur.execute("SELECT DISTINCT `" +
                                 row['Field'] + "` FROM " + i[0])
        ...
        if res['Create Table'].find("latin") >= 0:
            ...
            for line in lines:
                if line.find("latin") >= 0:
                    ...
                    rc = cur.execute("SELECT DISTINCT " +
                                     field + " FROM `" + table[0] + "`")
